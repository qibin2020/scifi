...
# SCEPCal optimization, epoch 4: follow the rear-length rise, test it against width

## Context

This is epoch 4 of my optimization campaign for the SCEPCal crystal
calorimeter and its readout. Epoch 3 found the score rising at the long end
of the rear section (best at 21 cm, the scan edge) while the projective
offset is flat. This epoch follows the rear-length rise beyond the edge and
simultaneously re-probes the crystal width there, in case a longer rear
section shifts the width optimum.

- Crystals **9, 11, 13, and 15 cm wide** × **rear lengths 19, 23, 27, and
  31 cm** (16 geometries).
- **Front length fixed at 16 cm**, **projective offset fixed at 10 cm** (it
  is flat — any value works, so keep the default).
...
- After the run is verified, scan the readout with the `bilevel_digi` skill
  using its **default grid**.

Call the run `cl4` (the skills place the config and outputs in this run's
...
`cl4`, then the `bilevel_ana` skill on run `cl4`. Each skill is one command
...
- The full-runner printed `RUN_VERIFIED run=cl4 roots=16` with a job id.
- The readout scan printed `DIGI_VERIFIED run=cl4`.
...
- `./analysis.md` exists with sections Best point, Trends, Readout,
  ...
  each swept geometry parameter (width and rear length); quotes the readout
