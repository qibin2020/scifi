...
- If all scanned parameters are FLAT → choose a geometry parameter that has not
  been scanned in any epoch yet and scan 4 values around its current value —
  which one and which values is your choice; justify it in one sentence in that
  epoch's report.
- Keep the reconstruction setup and event count unchanged. Each follow-up
  epoch scans exactly **4 geometries**.

Name the runs `aor1` … `aor5` (the skills place configs and outputs in this run's campaign area automatically; SLURM
...
- For each k in 1..5: the builder printed `CONFIG_VALIDATED` (geometries=6 for
  ...
  `RUN_VERIFIED run=aor<k>` with a job id, and `./analysis_<k>.md` exists
