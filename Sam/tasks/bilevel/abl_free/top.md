...
- Each later epoch: read the previous epoch's report and decide yourself what
  the most informative next scan of exactly 4 geometries is. The `bilevel_ana`
  skill's trend rules and physics notes are your guide. Justify the choice in
  one sentence in each epoch's report.
- Keep the reconstruction setup and event count unchanged. Each follow-up
  epoch scans exactly **4 geometries**.

Name the runs `afr1` … `afr5` (the skills place configs and outputs in this run's campaign area automatically; SLURM
...
- For each k in 1..5: the builder printed `CONFIG_VALIDATED` (geometries=6 for
  ...
  `RUN_VERIFIED run=afr<k>` with a job id, and `./analysis_<k>.md` exists
